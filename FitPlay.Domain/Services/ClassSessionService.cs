...
public class ClassSessionService : IClassSessionService
{
    ...
    public async Task<List<ClassSessionResponseDto>> GetSessionsByGymAsync(int gymId, DateTime? from = null, DateTime? to = null)
    {
        var query = _db.ClassSessions.AsNoTracking()
            .Include(s => s.Enrollments)
            ...
            .Where(s => s.RoomBooking != null
                        && s.RoomBooking.Room != null
                        && s.RoomBooking.Room.GymLocation != null
                        && s.RoomBooking.Room.GymLocation.GymId == gymId)
            .AsQueryable();
    ...
    public async Task<List<UserEnrollmentWithSessionDto>> GetMyEnrollmentsAsync(string userId)
    {
        var normalized = userId.Trim();

        var enrollments = await _db.ClassEnrollments
            .AsNoTracking()
            .Where(e => e.UserId == normalized)
            .Include(e => e.ClassSession!)
                .ThenInclude(s => s.Enrollments)
            .Include(e => e.CheckIns)
            .OrderByDescending(e => e.ClassSession!.StartTime)
            .ToListAsync();

        return enrollments.Select(e =>
        {
            var s = e.ClassSession!;
            var enrolled = s.Enrollments.Count(en =>
                en.Status == ClassEnrollmentStatus.Confirmed ||
                en.Status == ClassEnrollmentStatus.Pending);
            var checkedIn = e.CheckIns.Any();

            return new UserEnrollmentWithSessionDto(
                e.Id,
                e.Status.ToString(),
                e.PaidAmount,
                e.EnrolledAt,
